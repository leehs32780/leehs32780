...
const 그대로 = db
  .prepare("UPDATE 설비 SET 상태 = ? WHERE id = ?")
  .run("점검중", 1);
...
console.log({
  ...db.prepare("SELECT 이름, 상태, 온도 FROM 설비 WHERE id = 2").get(),
});
...
console.log(
  JSON.stringify(
    db
      .prepare("SELECT 이름, 온도 FROM 설비 WHERE 라인 = 'A'")
      .all()
      .map((설비) => `${설비.이름}:${설비.온도}`),
  ),
);
...
console.log(
  "NULL + 1 =",
  db.prepare("SELECT 온도 FROM 설비 WHERE 이름 = '온도없음'").get().온도,
);
// 출력: NULL + 1 = null
...
db.prepare("UPDATE 설비 SET 온도 = COALESCE(온도, 0) + 1 WHERE 이름 = ?").run(
  "온도없음",
);
console.log(
  "COALESCE 쓰면 =",
...
const 잘못받기 = db
  .prepare("UPDATE 설비 SET 온도 = ? WHERE id = ? RETURNING *")
  .run(60.0, 2);
console.log(잘못받기);
// 출력: { changes: 1, lastInsertRowid: 5 }
...
console.log(
  "처음:",
  db.prepare("SELECT 온도 FROM 설비 WHERE 이름 = '펌프 1호'").get().온도,
);
// 출력: 처음: 30
...
console.log(
  "줄 수:",
  db.prepare("SELECT COUNT(*) AS n FROM 설비 WHERE 이름 = '펌프 1호'").get().n,
);
// 출력: 줄 수: 1
...
시도("없는 상태값", () =>
  db.prepare("UPDATE 설비 SET 상태 = ? WHERE id = ?").run("수리중", 1),
);
// 출력: 없는 상태값 → 거절: CHECK constraint failed: 상태 IN ('정상', '점검중', '고장')
시도("겹치는 이름", () =>
  db.prepare("UPDATE 설비 SET 이름 = ? WHERE id = ?").run("프레스 1호", 1),
);
// 출력: 겹치는 이름 → 거절: UNIQUE constraint failed: 설비.이름

console.log({
  ...db.prepare("SELECT 이름, 상태 FROM 설비 WHERE id = 1").get(),
});
// 출력: { '이름': '용접로봇 1호', '상태': '정상' }
...
`);
소프트db.exec(
  "INSERT INTO 설비 (이름) VALUES ('용접로봇 1호'), ('프레스 1호')",
);
...
const 살아있는것 = 소프트db
  .prepare("SELECT 이름 FROM 설비 WHERE 삭제시각 IS NULL")
  .all();
...
console.log(
  "실제로는 남아 있음:",
  소프트db.prepare("SELECT COUNT(*) AS n FROM 설비").get().n,
);
// 출력: 실제로는 남아 있음: 2
...
시도("설비가 있는 라인 삭제", () =>
  관계db.prepare("DELETE FROM 라인 WHERE 코드 = ?").run("A"),
);
...
console.log(
  "설비도 사라짐:",
  연쇄db.prepare("SELECT COUNT(*) AS n FROM 설비").get().n,
);
// 출력: 설비도 사라짐: 0
