...
//
// ★★ 따옴표를 쓰면 대소문자가 그대로 남습니다. 그러면 **항상** 따옴표를 써야 합니다. // 중요 !!!
//
//   create table "Products" ("userName" text);
...
console.log(
  JSON.stringify(
    바꾸기({ id: 1, user_name: "김민준", created_at: "2026-08-18" }),
  ),
);
...
<<<<<<< HEAD:07_클라우드와배포/05_클라우드_DB_Supabase/개념02_Postgres는_무엇이_다른가.js
db.exec(
  "CREATE TABLE 설비 (id INTEGER PRIMARY KEY, name TEXT, line TEXT) STRICT",
);
=======
db.exec("CREATE TABLE 상품 (id INTEGER PRIMARY KEY, name TEXT, category TEXT) STRICT");
>>>>>>> 9867716458f8aa7c7d01a07d2cc319607de22c97:08_클라우드와배포/05_클라우드_DB_Supabase/개념02_Postgres는_무엇이_다른가.js
db.exec(`
...
console.log(
  "SQLite:",
  JSON.stringify(SQLite결과.map((행) => `${행.line}:${행.name}(${행.대수})`)),
);
...
//
<<<<<<< HEAD:07_클라우드와배포/05_클라우드_DB_Supabase/개념02_Postgres는_무엇이_다른가.js
//   SQLite  LIKE 는 영문 대소문자를 **안 가립니다** (02단원 개념03) // 같은지 여부를 물어본다
=======
...
console.log(
  ...
  JSON.stringify(
    db
      .prepare("SELECT id FROM 온도 ORDER BY 값")
      .all()
      .map((행) => 행.id),
  ),
);
...
console.log(
  ...
  JSON.stringify(
    db
      .prepare("SELECT id FROM 온도 ORDER BY 값 DESC")
      .all()
      .map((행) => 행.id),
  ),
);
...
console.log(
  ...
  JSON.stringify(
    db
      .prepare("SELECT id FROM 온도 ORDER BY 값 ASC NULLS LAST")
      .all()
      .map((행) => 행.id),
  ),
);
