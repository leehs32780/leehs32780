...
console.log(lines2[lines2.length - 2]);
...
fs.writeFileSync(path.join(__dirname, "메모.txt"), "이현수", "utf-8");

// 예를 들어 const 저장폴더 = path.join(__dirname, "uploads");
//
...
//
//   writeFileSync   덮어쓰기   (설정 파일 저장 등) // 중요 !!! //
//   appendFileSync  이어 쓰기  (접속 기록, 로그 등) // 중요 !!! //
//
...
//                    네 줄을 직접 써 보세요.
const f = path.join(__dirname, "연습6.txt");
fs.writeFileSync(f, "연습", "utf-8");
console.log(fs.existsSync(f));
fs.unlinkSync(f);
console.log(fs.existsSync(f));

// 현재 파일이 있는 폴더 위치와 "연습6.txt" 파일 이름을 합쳐서 안전한 파일 전체 경로를 만들어 f에 저장합니다.
// 위에서 만든 경로(f)에 "연습6.txt" 파일을 새로 만들고, 그 안에 "실습"이라는 글자를 적어서 저장합니다.
// 방금 만든 파일이 컴퓨터에 실제로 존재하는지 확인하고, 존재하므로 true를 출력합니다.
// 지정한 경로(f)에 있는 파일을 곧바로 삭제합니다.
// 파일이 삭제되어 더 이상 존재하지 않는지 확인하고, 없으므로 false를 출력합니다.
