...
app.get("/documents", (req, res) => {
  const sortOption = req.query.sort === "desc" ? "내림차순" : "기본";
...
//                    (확인했으면 반드시 되돌리세요)
// 답: req.body는 undefined가 됩니다.
//     JSON을 읽지 못해서

// ✏️ 직접 해보기 2 — app.use(express.json()) 을 맨 아래(listen 바로 위)로 옮겨 보세요.
//                    이번에는 어떻게 되나요? 왜 그럴까요?
// 답: 라우트가 먼저 실행되어 express.json()을 거치지 못하므로 req.body가 undefined가 됩니다.
// 좁은게 먼저 실행, 넓은게 나중에 실행
...
// 답: const tags = [].concat(req.query.tag || []);
